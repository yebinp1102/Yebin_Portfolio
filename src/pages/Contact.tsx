...
const logos = [
  ...
  {icon: Notion, link: 'https://www.notion.so/120f507aded64eff8a2f74641e571af2?pvs=4', text: 'Notion'},
    ...
    >
      ...
      <div className="max-w-7xl w-full mx-auto absolute top-0">
          ...
          <div className="absolute bg-pink-500 w-fit h-[250px] flex -top-4 rounded-md shadow-lg z-[10]">
            <h1 className="text-3xl font-bold flex self-end mb-10 px-10 tracking-wider
            ">
              Contact <br />Me
            </h1>
            ...
            <ul className="flex gap-6 mt-4">
              {logos.map(logo => (
                <li className="flex flex-col gap-1 items-center">
                  <a href={`${logo.link}`} target="_blank" className="w-11 h-11 bg-pink-500 rounded-full flex items-center justify-center">
                    <img src={logo.icon} alt="location" className="w-7" />
                  </a>
                  <p className="text-black-500 text-xs">{logo.text}</p>
                </li>
              ))}
            </ul>
          </div>

          <div className=" flex flex-col items-center justify-center bg-sub text-white m-10 rounded-lg shadow-lg px-10">
            <h3 className="text-3xl font-bold mb-16">Send me a message</h3>
            <form className="flex flex-col min-w-[400px] gap-12">
              <input placeholder="Name" className="flex-1 h-fit px-4 py-1 placeholder:text-sm placeholder:text-white/70 bg-transparent border-b border-white" />
              <input placeholder="E-mail address" className="flex-1 h-fit px-4 py-1 placeholder:text-sm placeholder:text-white/70 bg-transparent border-b border-white" />
              <input placeholder="Message" className="flex-1 h-fit px-4 py-1 placeholder:text-sm placeholder:text-white/70 bg-transparent border-b border-white" />
            </form>
            <button className="bg-white text-sub px-10 py-1.5 rounded-lg shadow-lg font-semibold self-end mt-16 hover:scale-105 transition">
              전송하기
            </button>
          </div>
        </div>
